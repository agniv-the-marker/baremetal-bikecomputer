...
// experimental dma! mode 1 is per block dma.
void emmc_dma_enable(int on);

// 0=PIO, 1=per-block DMA, 2=whole-transfer DMA (one DREQ-paced DMA per flush,
// busy-wait), 3=whole-transfer + completion IRQ + wfi (needs dma_irq_init()).
void emmc_dma_set_mode(int mode);
